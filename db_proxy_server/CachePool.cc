...
int CacheConn::Init()
{
	...
	if (!m_pContext || m_pContext->err) {
		if (m_pContext) {
			LOG_ERROR("redisConnect failed: %s", m_pContext->errstr);
			redisFree(m_pContext);
			m_pContext = NULL;
		} else {
	...
	if (reply && (reply->type == REDIS_REPLY_STATUS) && (strncmp(reply->str, "OK", 2) == 0)) {
		...
		LOG_ERROR("select cache db failed");
		return 2;
	}
...
string CacheConn::get(string key)
{
	...
	if (!reply) {
		LOG_ERROR("redisCommand failed:%s", m_pContext->errstr);
		redisFree(m_pContext);
		m_pContext = NULL;
		return value;
	}
...
long CacheConn::incrBy(string key, long value)
{
    ...
    if(!reply)
    {
        LOG_ERROR("redis Command failed:%s", m_pContext->errstr);
        redisFree(m_pContext);
        m_pContext = NULL;
        return -1;
    }
...
int CachePool::Init()
{
	...
	LOG_ERROR("cache pool: %s, list size: %lu", m_pool_name.c_str(), m_free_list.size());
	return 0;
}
	...
	while (m_free_list.empty()) {
		if (m_cur_conn_cnt >= m_max_conn_cnt) {
			...
			if (ret) {
				...
				m_cur_conn_cnt++;
				LOG_ERROR("new cache connection: %s, conn_cnt: %d", m_pool_name.c_str(), m_cur_conn_cnt);
			}
		}
	}
...
int CacheManager::Init()
{
	...
	for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
		...
		if (!cache_host || !str_cache_port || !str_cache_db || !str_max_conn_cnt) {
			LOG_ERROR("not configure cache instance: %s", pool_name);
			return 2;
		}
