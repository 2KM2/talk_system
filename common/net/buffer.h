...
{
public:
    static const size_t kCheapPrepend = 8;//可以放这次解析的长度
...
private:
    ...
    {
        /**
         * kCheapPrepend     |readIndex  writelen
         * kCheapPrepend| len                     |
         * 如果从 kCheapPrepend-readerIndex + writableBytes() 小于 需要得 长度，resize
        */
        if (writableBytes() + prependableBytes() < len + kCheapPrepend)
        {
            buffer_.resize(writerIndex_ + len);//后面补上
        }
        else
        {
            //数据往前移,腾出空间
