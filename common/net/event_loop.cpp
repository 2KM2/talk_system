...
//保证每个线程只有一个EventLoop
__thread CEventLoop *t_loopInThisThread = nullptr;
...
/**
 * mainLoop bypass wakeupfd to notify subloop
*/
...
void CEventLoop::loop()
{
    ...
    while(!quit_)
    {
        ...
        for (CChannel *channel : activeChannels_)
        {
        ...
        /**
         * IO线程 mainLoop accept fd <<=channel  subloop(3)
...
{
  uint64_t one = 1;
  ssize_t n = read(wakeupFd_, &one, sizeof(uint64_t));
  if (n != sizeof one)
  {
